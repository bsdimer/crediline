...
@Service("streetImporter")
public class StreetImporter extends AbstractDataImporter implements DataImporter {
    ...
    public DataImporter parse(InputStream stream) throws IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                collection.add(new RowMapper(Street.class, values));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public DataImporter persist() {
        for (RowMapper rowMapper : getCollection()) {
            String query = "insert into streets set ";
