 ...
 */
public class CreditInProgressSpecification extends AbstractSpecification<Credit> {

    @Override
    public Predicate toPredicate(PotentialJoin<?, Credit> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        return cb.equal(root.get().get(Credit_.creditState), CreditState.IN_PROGRESS);
    }
}
