...
var attributes = {id: "qz", code: 'qz.PrintApplet.class', width: 1, height: 1};
var parameters = {jnlp_href: './resources/qzprint-applet/qz-applet-0.1.jnlp'};


/*deployQZHere();*/

function deployQZHere() {
...
function deployQZ(container) {
    /*if (deployJava.versionCheck("1.7+") == true) {
     }
     else if (deployJava.versionCheck("1.6+") == true) {
     delete parameters['jnlp_href'];
     }*/
    docWriteWrapper($('#' + container), function () {
...
function printPDF(file) {
    ...
    qz.appendPDF(getPath() + "resources/misc/" + file);
 ...
 *    qz.printPS();
 ***************************************************************************/
function printHTML(source) {
    if (notReady()) {
        return;
    }
    qz.appendHTML('<html>' + source + '</html>');
    qz.printHTML();
}
